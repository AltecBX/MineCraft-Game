...
```
npm run check                                        # syntax of gfx.js and every src file
...
```
index.html   shell, loads three.min.js (CDN), gfx.js, then src/ in order
styles.css   all CSS
gfx.js       procedural textures, shaders and the atmosphere model
src/         the game split by system into ordered classic scripts (load order in src/FILES)
...
This started as one HTML file built in seven stages plus an audio and accessibility pass. It was split into index.html, styles.css and game.js for editing in Claude Code, and later game.js was split by system into the ordered scripts in src/, which still need no build step.
